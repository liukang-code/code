...
/**
 * @author LiuKang on 2021/5/20 16:13
 */
@Slf4j
@Component
...
public class DefaultAlipayServiceImpl implements AlipayServiceApi {
    ...
    public AlipayResultVO getOrderStatus(String orderId) {
        boolean orderStatus = tradeQuery(orderId);
        if (orderStatus){
            AlipayResultVO alipayResultVO = new AlipayResultVO();
            alipayResultVO.setOrderId(orderId);
            alipayResultVO.setOrderStatus(1); //1-支付成功
            alipayResultVO.setOrderMsg("订单支付成功");
    ...
    public String tradePrecreate(String orderId) {
        ...
        String body = "购买电影票共花费" + orderInfo.getOrderPrice() + "元";

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "lk";
...
//                dumpResponse(response);   //打印

                // 需要修改为运行机器上的路径
                filePath = String.format("D:\\Users\\laod\\IdeaProjects\\naan1993-guns-master\\guns\\guns-alipay\\src\\main\\resources/img/qr-%s.png", response.getOutTradeNo());
