...
import com.stylefeng.guns.rest.common.persistence.dao.MoocOrder2021TMapper;
...
public class Order2021ServiceImpl implements OrderServiceApi {
...
//    private FTPUtil ftpUtil;
    ...
    @Autowired
    private MoocOrder2021TMapper moocOrder2021TMapper;
...
//          根据filedId找到
        String seatPath = moocOrder2021TMapper.getSeatsByFieldId(fieldId);
//        String address = ftpUtil.getFileStringByAddress(seatPath);
    ...
    public boolean isNotSoldSeats(String fieldId, String seats) {
        ...
        List<MoocOrder2021T> list = moocOrder2021TMapper.selectList(entityWrapper);
        String[] seatArrs = seats.split(",");
//          有任何一个编号匹配上，则直接返回失败
        for (MoocOrder2021T moocOrder2021T : list) {
            String[] ids = moocOrder2021T.getSeatsIds().split(",");
            for (String id : ids) {
                for (String seat : seatArrs) {
...
//        插入订单实体
        MoocOrder2021T moocOrder2021T = new MoocOrder2021T();
        moocOrder2021T.setUuid(uuid);
        moocOrder2021T.setSeatsName(seatsName);
        moocOrder2021T.setSeatsIds(soldSeats);
        moocOrder2021T.setOrderUser(userId);
        moocOrder2021T.setOrderPrice(totalPrice);
        moocOrder2021T.setFilmPrice(filmPrice);
        moocOrder2021T.setFilmId(filmId);
        moocOrder2021T.setFieldId(filedId);
        moocOrder2021T.setCinemaId(cinemaId);

        Integer result = moocOrder2021TMapper.insert(moocOrder2021T);
        if (result > 0) {
            OrderVO orderVO = moocOrder2021TMapper.getOrderInfoById(uuid);
    ...
    public Page<OrderVO> getOrderInfoByUserId(Integer userId, Page<OrderVO> page) {
        ...
        List<OrderVO> orders = moocOrder2021TMapper.getOrdersInfoByUserId(userId);
        if (orders == null || orders.size() == 0) {
            result.setTotal(0);
            result.setRecords(new ArrayList<>());
            return result;
        } else {
            EntityWrapper<MoocOrder2021T> entityWrapper = new EntityWrapper<>();
            entityWrapper.eq("order_user", userId);
            Integer count = moocOrder2021TMapper.selectCount(entityWrapper);
    ...
    public String getSoldSeatsByFiledId(Integer filedId) {
        if (filedId == null) {
            ...
            String seats = moocOrder2021TMapper.getSoldSeatsByFieldId(filedId);
            return seats;
        }
    }
    ...
    public OrderVO getOrderInfoById(String orderId) {
        return moocOrder2021TMapper.getOrderInfoById(orderId);
    }

    @Override
    public boolean paySuccess(String orderId) {
        MoocOrder2021T order2021T = new MoocOrder2021T();
        order2021T.setUuid(orderId);
        order2021T.setOrderStatus(1);
        Integer result = moocOrder2021TMapper.updateById(order2021T);
        if (result > 0) {
            return true;
        } else {
            return false;
